...
params:
  ...
  dataPackage1RefID: 2311566
  dataPackage1Title: "Mojave Desert Network (MOJN) Streams and Lakes (STLK) Data Package (2009-2024)"               # Should match title in data store.
  dataPackage1Description: "MOJN STLK Data Package 2009-2024"  
  dataPackage2RefID: 2307285                              # Data Store reference ID for data set associated with this report. You must have at least one.
  dataPackage2Title: "Mojave Desert Benthic Macroinvertebrate (BMI) Data Package (2009-2023)"               # Should match title in data store.
  dataPackage2Description: "MOJN BMI Data Package 2009-2023"  
...
pagetitle: "MOJN STLK QC 2024"
lang: "en"
title: |
    | Mojave Desert Network Streams and Lakes Protocol
    | Data Quality Report 2024 `r params$reportNumber`        
subtitle: |
  | Data Package 2024  
author:
...
# Data Records
The published product includes two data packages containing multiple CSV data files:

* **`r params$dataPackage1Title`.** Comma-separated text files containing discrete data for the MOJN I&M Streams and Lakes protocol. These data were compiled by MOJN I&M and were last updated on May 14, 2025. Available at `r paste0("https://irma.nps.gov/DataStore/Reference/Profile/", params$dataPackage1RefID)`.
* **`r params$dataPackage2Title`.** Comma-separated text files containing benthic macroinvertebrate data for all MOJN I&M protocols. These data were compiled by MOJN I&M and were last updated on December 18, 2024. Available at `r paste0("https://irma.nps.gov/DataStore/Reference/Profile/", params$dataPackage2RefID)`.

The streams and lakes data package associated with this report contains twelve CSV files contain raw values suitable for use with the MOJN I&M Streams and Lakes R package. Three additional CSV files contain calculated values generated using this R package. These files are clearly labeled as "calculated" (Table 1). A data dictionary for all of the tables and fields in the published CSV files is provided in XML format with the streams and lakes data package.

The benthic macroinvertebrate data package contains three CSV files along with a data dictionary provided in XML format with the data package.
...
FileName <- c("Site","Visit","Channel","Chemistry","Clarity","LakeLevelString","LakeLevelSurvey","WaterQualityDO","WaterQualitypH","WaterQualitySpCond","WaterQualityTemperature","WQStreamXSection","LakeLevel_CALCULATED","LakeWaterQuality_CALCULATED","StreamWaterQuality_CALCULATED")
Description <- c("Description of each site code used in this dataset (Streams and Lakes)","General attributes and information for each annual site visit (Streams and Lakes)","Channel characteristics associated with benthic macroinvertebrate samples (Streams)","Water chemistry sample laboratory results (Streams and Lakes)","Secchi depth measurements (Lakes)","Lake level measured using a string-based survey","Lake level measured using a digital level-based survey","Dissolved oxygen measured in situ through a depth profile (Lakes)","pH measured in situ through a depth profile (Lakes)","Specific conductance measured in situ through a depth profile (Lakes)","Temperature measured in situ through a depth profile (Lakes)","In situ water quality measured through stream cross sections (Streams)","Lake surface elevation calculated from the raw data in LakeLevelString and LakeLevelSurvey using the MOJN Streams and Lakes R package (Lakes)","Median water quality depth profile values calculated from the four raw WaterQuality data tables (Lakes)","Median water quality values calculated from the raw values in the WQStreamXSection data table (Streams)")
...
This data QC report focuses primarily on data collected in WY2024. These data have been summarized and plotted with previously published years of data in order to visualize the data in a broader context and detect outliers. The CSV data files accompanying this report are comprehensive of all years of data from WY2009-2022. The decision to publish a comprehensive dataset was intended to promote data usability by providing all years of data in one set of CSVs with consistent formatting.

## Advice and Caveats
- Monitoring at Dead Lake and Brown Lake began in 2010. Monitoring at all other lakes began in 2009.
- Johnson Lake and Dead Lake were not monitored in 2013 due to the lapse in federal appropriations.
- In 2009 and 2010, Baker Creek (South Fork) was visited for annual benthic macroinvertebrate and water chemistry sampling. In 2011 and later years, Baker Creek (Main Stem) was visited for sampling.
- In 2015, 2021, and 2022, no data were collected at Dead Lake because the lake was entirely dry.
- In 2021, the Utah State University National Aquatic Monitoring Center (USU NAMC) revised their benthic macroinvertebrate sample reporting methods. They began referring to abundance metrics as density metrics and made changes to their operational taxonomic units (OTUs). USU NAMC regenerated all previous sample reports, which were integrated into the WY2009-2021 streams and lakes dataset.
- In 2024, USU NAMC revised their benthic macroinvertebrate sample reporting methods. They began reporting unique richness instead of OTU standardized richness. USU NAMC regenerated all previous sample reports, which were integrated into the WY2009-2023 benthic macroinvertebrate dataset. As a result, there may be discrepancies in metrics values between the earlier WY2009-2023 streams and lakes dataset and the newer WY2009-2023 benthic macroinvertebrate dataset.
- In 2021, MOJN I&M standardized the flags applied to water chemistry data that did not meet certain standards or expectations (e.g., below minimum detection level).
- In 2022, the Oregon State University Cooperative Chemical Analytical Laboratory (OSU CCAL) reported NO3-N concentrations instead of NO3NO2-N due to MOJN I&M error. Because nitrite is usually zero in aerobic samples, the NO3-N concentrations should be very close to NO3NO2-N.
- In 2024, the sample freezer used to store water chemistry samples after collection broke down. Samples may have thawed for a few days to a few weeks before they were refrozen and shipped to OSU CCAL. It is not know what effect this may have had on sample quality.
...
- In the WY2009-2021 and earlier datasets, water chemistry data for Strawberry Creek and Snake Creek in 2012 were switched. These data are now attributed to the correct stream in the WY2009-2022 and later datasets.
...
`r if(as.numeric(params$wateryearStart) < 2018) {"These are the benchmark elevations for each lake and field season using the string survey method. Count refers to number of readings collected for each benchmark, with the mean and standard deviation summarized for all counts. The reference mark designated Benchmark 1 was assigned a given elevation of 100 m (328.084 ft). String surveys ended in 2018."}`

``` {r string.elevations, fig.cap = capTab("Calculated elevations for each lake water surface from 2010 until the string method was last used in 2018.")}
#| include = as.numeric(params$wateryearStart) < 2018

string.elevations <- StringSurveyElevation(field.season = year.range)

string.elevations$Elevation_ft <- round(string.elevations$Elevation_ft, 3)
string.elevations$StDev_ft <- round(string.elevations$StDev_ft, 3)
...
### Digital level elevations
These are the benchmark elevations for each lake and field season using the digital level method, which began in 2018. The closure error applies to all benchmarks in the same circuit. The reference mark designated Benchmark 1 was assigned a given elevation of 100 m (328.084 ft).
...
benchmark.elevations <- SurveyPointElevation(field.season = year.range)

benchmark.elevations$ClosureError_ft <- round(benchmark.elevations$ClosureError_ft, 4)
benchmark.elevations$Elevation_ft <- round(benchmark.elevations$Elevation_ft, 3)
...
This table shows the variability in benchmark elevations by method for each lake. Count indicates the number of years of data, with the mean and standard deviation summarized for all counts. The reference mark designated Benchmark 1 was assigned a given elevation of 100 m (328.084 ft).
...
benchmark.qc <- qcBenchmarkConsistency(field.season = year.range)

benchmark.qc$MeanElevation_ft <- round(benchmark.qc$MeanElevation_ft, 3)
benchmark.qc$StDev_ft <- round(benchmark.qc$StDev_ft, 3)
...
### Benchmark elevations plot
This is a plot of the elevations for each benchmark across field seasons for each lake. Consistent upward or downward trends in the elevation of a benchmark may indicate that the benchmark is unstable. An abrupt change in the elevation of a benchmark may indicate disturbance at the location of the benchmark, incorrect positioning of the string or rod on the benchmark, or error introduced by wind. Use of the digital level began in 2018, and string surveys ended in 2018. The digital level method improved accuracy and precision compared to the earlier string survey method.
...
benchmark.plots <- PlotBenchmarkElevation(field.season = year.range)

benchmark.plots # ggplotly does not support complext legends

# if (params$isAccessible == "yes") {
#   benchmark.plots
# } else {
#   benchmark.plots <- plotly::ggplotly(benchmark.plots, tooltip = "text") |>
#     layout(legend = list(orientation = "h", x = 0, y = 0))
#   
#   for (i in 1:length(benchmark.plots$x$data)){
#     if (!is.null(benchmark.plots$x$data[[i]]$name)){
#       benchmark.plots$x$data[[i]]$name =  gsub("\\(","",str_split(benchmark.plots$x$data[[i]]$name,",")[[1]][1])
#     }
#   }
#   benchmark.plots
# }
...
This is a plot of lake surface elevations as determined by all benchmarks across field seasons where the string method was used. In cases where Benchmark 1 is an outlier, another benchmark may be selected as the authoritative reference mark to calculate lake surface elevation. 

``` {r string.compare.plots, warning = FALSE, fig.cap = capFig("Benchmark elevations."), fig.alt = "Line graphs of benchmark elevations at each lake since 2018, with field season on the x-axis and elevation in feet on the y-axis."}
string.compare <- PlotStringComparisons(field.season = year.range)

if (params$isAccessible == "yes") {
  string.compare
} else {
  plotly::ggplotly(string.compare, tooltip = "text") |>
    layout(legend = list(orientation = "h", x = 0, y = -0.15))
}
```

### Lake surface elevations
These are the water surface elevations for each lake and field season reported in meters and feet.

``` {r lake.elevations, warning = FALSE, fig.cap = capTab("Calcuated lake water surface elevations.")}
lake.elevations <- LakeSurfaceElevation(field.season = year.range)

lake.elevations$Elevation_ft <- round(lake.elevations$Elevation_ft, 3)
lake.elevations$Elevation_m <- round(lake.elevations$Elevation_m, 3)
...
### Lake surface elevations plot
This is a plot of water surface elevations across field seasons for each lake. Dead Lake was dry in 2015, 2021, and 2022. Dead Lake and Johnson Lake were not visited in 2013 due to the lapse in federal appropriations. Dead Lake and Teresa Lake exhibit the greatest annual variability, while Johnson Lake and Baker Lake exhibit the least annual variability.
...
lake.elev.plots <- PlotLakeSurfaceElevation(field.season = year.range)
...
depths.exceed.lake <- qcSecchiGTDepth(field.season = year.range)
...
lake.dry.depths.exist <- qcLakeDryMeasurementsExist(field.season = year.range)
...
lake.not.dry.no.clarity <- qcLakeNotDryMeasurementsMissing(field.season = year.range)
  ...
  reactable::reactable(
    sortable = TRUE,
      ...
      defaultColDef = reactable::colDef(
        header = function(value) snakecase::to_title_case(value)))
```
There are `r nrow(lake.not.dry.no.clarity)` records with missing data. For these records, lake depth (depth to bottom) was not measured because Secchi depth was measured instead.
...
lake.not.dry.no.depths <- qcSecchiDepthMissing(field.season = year.range)
...
wq.stream.sanity <- qcStreamWqSanity(field.season = year.range)

wq.stream.sanity %<>%
  dplyr::select(-c(Park, VisitType)) %>%
...
wq.lake.sanity <- qcLakeWqSanity(field.season = year.range)
...
wq.stream.flags <- qcStreamWqFlags(field.season = year.range)
...
wq.lake.flags <- qcLakeWqFlags(field.season = year.range)
...
temp.dp.plot <- WqPlotTemperatureDepthProfile(field.season = year.range)
...
ph.dp.plot <- WqPlotPHDepthProfile(field.season = year.range)
...
sc.dp.plot <- WqPlotSpCondDepthProfile(field.season = year.range)
...
do.dp.plot <- WqPlotDODepthProfile(field.season = year.range)
...
chem.flags <- qcChemFlags(field.season = year.range)
...
lab.dupes <- qcChemLabDupes(field.season = year.range)
...
field.dupes <- qcChemFieldDupes(field.season = year.range)
...
field.blanks <- qcChemFieldBlanks(field.season = year.range)
...
chem.TDN <- qcChemTDN(field.season = year.range)
...
reactable::reactable(dplyr::select(chem.TDN, -TDNFlag),
      ...
      defaultColDef = reactable::colDef(
        ...
        header = function(value) if (value %in% c("TDN", "UTN")) {
          value
        } else if (value %in% c("Difference")) {
          "Difference"
        } else {
          snakecase::to_title_case(value)
      }))
```
Of the `r nrow(chem.TDN)` records where the concentration of dissolved nitrogen is greater than the concentration of total nitrogen, there are `r nrow(chem.TDN %>% filter(Difference >= 0.02))` records where the difference is outside the normal limits of variability.

### Nitrate and nitrite
These are the records where the concentration of nitrate (NO3) and nitrite (NO2) exceeds either total nitrogen (UTN) or total dissolved nitrogen (TDN). If the discrepancy falls within precision limits, this indicates that nearly all of the nitrogen or dissolved nitrogen is in the form of nitrate and nitrite, and the two concentrations are essentially equal to each other. If the discrepancy falls outside of precision limits, this may indicate contamination of the sample. Alternatively, this may be a consequence of the natural heterogeneity of the water body. The total nutrient concentration is measured from the unfiltered sample, while the dissolved nutrient concentration (including nitrate and nitrite) is measured from the filtered sample. These two samples are collected at approximately the same time and location, but there may be some variability that becomes more evident at low concentrations. Whether the discrepancy falls within or outside precision limits is documented in the notes field. In 2022, NO3 concentrations were reported instead of NO3 and NO2. Because nitrite is usually zero in aerobic samples, the NO3 concentrations should be very close to the total of NO3 and NO2.
...
chem.NO3NO2 <- qcChemNO3NO2(field.season = year.range)
...
chem.TDP <- qcChemTDP(field.season = year.range)
...
lookup <- getMDLLookup()
chem.MDL <- qcChemMDL(field.season = year.range)
...
lookup <- getMDLLookup()
chem.ML <- qcChemML(field.season = year.range)
...
chem.stream.nutrient.plot <- ChemStreamNutrientPlot(field.season = year.range)
...
chem.stream.ion.plot <- ChemStreamIonPlot(field.season = year.range)
...
chem.lake.nutrient.plot <- ChemLakeNutrientPlot(field.season = year.range)
...
chem.lake.ion.plot <- ChemLakeIonPlot(field.season = year.range)
...
bmi.discrepancies <- qcBMIDiscrepancies(field.season = year.range)
...
bmi.gen.plot <- BMIGeneralMetricsPlot(field.season = year.range)
...
bmi.div.plot <- BMIDiversityMetricsPlot(field.season = year.range)
...
Groups are based on sensitivity to disturbance or pollution. The Hilsenhoff Biotic Index (HBI) is the abundance-weighted average of family-level pollution tolerances, and is plotted in the diversity metrics section. Tolerant taxa have an HBI score >=8 and are able to withstand disturbance and poor water quality. Intolerant taxa have an BMI score <=2 and are not as able to withstand disturbance and poor water quality. Long-lived taxa have 2- to 3-year life cycles and can indicate habitat stability and persistent good water quality in their habitat.
...
bmi.tol.plot <- BMISensitivityMetricsPlot(field.season = year.range)
...
Groups are based on functional feeding groups, which refer to the type of food resource that species utilize in the stream.  Functional feeding group descriptions come from A Guide to Common Freshwater Invertebrates of North America (Voshell, 2002).

- Collector-filterer taxa consume fine particulate organic matter (FPOM) suspended in the water column.
- Collector-gatherer taxa consume FPOM from the benthic substrate.
- Parasites feed on a host organism.
- Piercer herbivore taxa feed through piercing plant tissues and extracting the liquid contents.
- Predator taxa consume living animal tissue, usually other aquatic invertebrates, but occasionally small vertebrates.
- Scraper taxa remove and consume the thin layer of algae that grows on solid surfaces in shallow water. 
- Shredder taxa consume living or decomposing vascular plant tissue and coarse particulate organic matter (CPOM).
...
bmi.fun.plot <- BMIFunctionalMetricsPlot(field.season = year.range)
...
Groups are based on habits, which refer to locomotion mode preferred by species in the stream. Habit descriptions come from A Guide to Common Freshwater Invertebrates of North America (Voshell, 2002).

- Burrower taxa tunnel into the fine sediment substrate or occasionally into plant stems, leaves, or roots.
- Climber taxa maneuver up and down the stems and leaves of submerged plants, roots, and woody debris.
- Clinger taxa attach themselves to coarse substrate, sturdy plants, woody debris, or other firm surfaces in moving water.
- Crawler taxa move around slowly in small, protected places among loose stones, woody debris, coarse detritus, exposed roots, and plant bases.
- Planktonic taxa lack mobility and float in the water column at the direction of water currents.
- Skater taxa skim or stride on the water surface using surface tension.
- Sprawler taxa inhabit the surface of the fine sediment substrate.
- Swimmer taxa actively move themselves through the water column by using either a fishlike or a paddling motion.

``` {r bmi.hab.plot, fig.width = 8, warning = FALSE, fig.cap = capFig("Stream benthic macroinvertebrate functional feeding group metrics."), fig.alt = "Line graphs of stream benthic macroinvertebrate functional feeding group metrics, with field season on the x-axis and count on the y-axis. Each of the nine streams has a vertical panel, and richness and density each have a horizontal panel. Counts for the five functional feeding groups are represented by different line colors."}
bmi.hab.plot <- BMIHabitMetricsPlot(field.season = year.range)

if (params$isAccessible == "yes") {
  bmi.hab.plot
} else {
  ggplotly(bmi.hab.plot,
           tooltip = "text") %>%
    layout_ggplotly(x = -0.02, y = -0.03)
}
```
...
Groups are based on taxonomic levels. The insect orders Ephemeroptera (mayflies), Plecoptera (stoneflies), and Tricoptera (caddisflies) are together often referred to as EPT taxa. Insects in these orders tend to be more sensitive to disturbance, and their presence and abundance can indicate good water quality.
...
bmi.tax.plot <- BMITaxonomicMetricsPlot(field.season = year.range)
...
# References
Caudill CC, Moret G, Chung-MacCoubrey A, Baker G, Tallent N, Hughson D, Burke J, Starcevich LH, Steinhorst R. 2012. Mojave Desert Network Inventory and Monitoring streams and lakes monitoring protocol: Protocol narrative version 1.0. Natural Resource Report. NPS/MOJN/NRR—2012/593. National Park Service. Fort Collins, Colorado. Available at: https://irma.nps.gov/DataStore/Reference/Profile/2190889
...
Moret G, Caudill CC, Burke J. 2012. Mojave Desert Network Inventory and Monitoring streams and lakes protocol: Standard operating procedures and supplementary materials version 1.0. Natural Resource Report. NPS/MOJN/NRR—2012/593.1. National Park Service. Fort Collins, Colorado. Available at: https://irma.nps.gov/DataStore/Reference/Profile/2190896
...
Voshell, JR. 2002. A Guide to Common Freshwater Invertebrates of North America. The McDonald & Woodward Publishing Company: Granville, Ohio.
