...
This data release report focuses primarily on data collected in WY2021. These data have been summarized and plotted with previously published years of data (WY2018 to WY2020) in order to visualize the data in a broader context and detect outliers. The  tabular dataset accompanying this report is comprehensive of all years of data from WY2009 to WY2021. The decision to publish a comprehensive dataset was intended to promote data usability by providing all years of data in one set of CSVs with consistent formatting.

## Advice and Caveats
- In 2021, no data were collected at Dead Lake because the lake was entirely dry.
- In 2022, the Utah State University National Aquatic Monitoring Center (USU NAMC) revised its benthic macroinvertebrate sample reporting methods. They now refer to abundance metrics more accurately as density metrics. In addition, they now use unique taxonomy instead of applying a generic operational taxonomic unit (OTU) scheme. USU NAMC regenerated all previous sample reports, which have been integrated into this data release. As a result, there may be minor discrepancies in metrics values between the WY2009-2020 dataset and this WY2021 dataset.
...
This plot shows richness (number of taxa within a group) and abundance (number of individuals per m2 within a group) for stream BMI samples.
