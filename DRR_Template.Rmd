...
``` {r benchmark.plots, warning = FALSE, fig.alt = "Line graphs of benchmark elevations at each lake since 2018, with field season on the x-axis and elevation in feet on the y-axis. Benchmarks generally appear stable."}
...
``` {r lake.elev.plots, warning = FALSE, fig.alt = "Line graphs of lake surface elevations at each lake since 2010, with field season on the x-axis and elevation in feet on the y-axis. Lake surface elevations fluctuate each year, with Dead Lake and Teresa Lake exhibiting the highest fluctuations."}
...
reactable(dplyr::select(wq.stream.sanity, -FlagNote),
          ...
          details = colDef(
            ...
            details = function(index) {
              wq.stream.sanity$FlagNote[index]
            }
          ),
          resizable = TRUE,
          compact = TRUE)
...
reactable(dplyr::select(wq.stream.flags, -FlagNote),
          ...
          details = colDef(
            name = "Notes",
            details = function(index) {
              wq.stream.flags$FlagNote[index]
            }
          ),
          resizable = TRUE,
          compact = TRUE)
...
``` {r temp.dp.plot, warning = FALSE, fig.alt = "Plot of lake water temperature values, with field season on the x-axis and measurement depth in meters on the y-axis. Marker color indicates water temperature in degrees Celsius. Lake water temperature values vary by year and are typically between the range of 2.5 to 22.5 degrees Celsius. Lakes generally have a similar temperature throughout the water column, though they often have a relatively warmer temperature at the surface."}
...
``` {r ph.dp.plot, warning = FALSE, fig.alt = "Plot of lake pH values, with field season on the x-axis and measurement depth in meters on the y-axis. Marker color indicates pH. Lake pH values vary slightly by year and are typically basic between the range of 7 to 9.5. Lakes generally have a similar pH throughout the water column, though acidity occasionally increases with depth."}
...
``` {r sc.dp.plot, warning = FALSE, fig.alt = "Plot of lake specific conductance values, with field season on the x-axis and measurement depth in meters on the y-axis. Marker color indicates specific conductance in micro-Siemens per centimeter. Lake specific conductance values vary slightly by year and are typically less than 50 micro-Siemens per centimeter, although Brown Lake in 2020 was as high as 162 micro-Siemens per centimeter. Lakes generally have a similar specific conductance throughout the water column."}
...
``` {r do.dp.plot, warning = FALSE, fig.alt = "Plot of lake dissolved oxygen values, with field season on the x-axis and measurement depth in meters on the y-axis. Marker color indicates dissolved oxygen in milligrams per liter. Lake dissolved oxygen values vary slightly by year and are typically between the range of 6 to 9 milligrams per liter, with some slightly higher values at Teresa Lake in 2014 and lower values at Dead Lake in 2017. Lakes generally have a similar dissolved oxygen throughout the water column. Occasionally there is very low dissolved oxygen at the bottom, likely due to the probe resting on anoxic substrate."}
